...
public class GetReceipt extends HttpServlet {
            ...
            throws ServletException, IOException, SQLException, ClassNotFoundException, DoesntExistException 
    {
        ...
        int receiptId=dataObject.optInt("orderId");
        OrderHistorySQLCommands order=new OrderHistorySQLCommands(connection);
        Receipt receipt=order.getOrderReceipt(receiptId);
        OrderJsoner orderJsoner=new OrderJsoner();
        JSONObject receiptObject=orderJsoner.getReceiptJson(receipt);
        JSONObject responseObject=responser.response(receiptObject, 1);

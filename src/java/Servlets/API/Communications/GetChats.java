...
public class GetChats extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet GetChats</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet GetChats at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
...
//        JSONObject dataObject=new JSONObject(data);
        ...
        synchronized(comSQL)
        {
            chats=comSQL.getChats(user.getUser().getUserId());
            comSQL.markReadChats(user.getUser().getUserId());
        }
        CommunicationJsoner comJsoner=new CommunicationJsoner();
        JSONArray chatArray=comJsoner.getChatsArray(chats);

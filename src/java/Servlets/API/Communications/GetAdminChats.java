 ...
 * @author Moses
 */
public class GetAdminChats extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet GetAdminChats</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet GetAdminChats at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
...
//         Admin admin=adminsafe.getAdmin();
        ...
        synchronized(comSQL)
        {
            chats=comSQL.getChats(dataObject.optInt("userId"));
            comSQL.markReadChats(dataObject.optInt("userId"));
        }
        CommunicationJsoner comJsoner=new CommunicationJsoner();
        JSONArray chatArray=comJsoner.getAdminChatsArray(chats);
        
        out.println(responser.response(chatArray, 1));

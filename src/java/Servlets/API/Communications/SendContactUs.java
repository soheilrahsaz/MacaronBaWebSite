 ...
 * @author Moses
 */
public class SendContactUs extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet SendContactUs</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet SendContactUs at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
...
//        }
        ...
        Message message=comObjector.getMessageObject(dataObject);
        CommunicationSQLCommands comSQL=new CommunicationSQLCommands(connection);
        
        if(user.getLogged())
        {
            comSQL.addMessage(user.getUser().getUserId(), message.getText());
        }else
        {
            if(Methods.Methods.isNullOrEmpty(message.getName(),message.getPhoneNumber()))
            {
                out.println(responser.response(10));
                return;
            }
            comSQL.addMessage(message.getName(), message.getPhoneNumber(), message.getText());
        }

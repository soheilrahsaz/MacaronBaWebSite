...
public class GetChat extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet GetChat</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet GetChat at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
...
//        JSONObject dataObject=new JSONObject(data);
        ...
        synchronized(comSQL)
        {
            chats=comSQL.getNewChats(user.getUser().getUserId());
            comSQL.markReadChats(user.getUser().getUserId());
        }
        
        CommunicationJsoner comJsoner=new CommunicationJsoner();
        JSONArray chatsArray=comJsoner.getChatsArray(chats);

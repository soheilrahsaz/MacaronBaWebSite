...
public class SendChat extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet SendChat</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet SendChat at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
            ...
            throws ServletException, IOException, SQLException, ClassNotFoundException, DoesntExistException 
    {
        ...
        if(!comSQL.isChatFree(user.getUser().getUserId()))
        {
        ...
        if(chat.getText().length()>=256)
        {
            out.println(responser.response(20));
            return;
        }
        
        comSQL.addChat(user.getUser().getUserId(), chat.getText(), 0,1,0);

...
public class SendComment extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet SendComment</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet SendComment at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
            ...
            throws ServletException, IOException, SQLException, ClassNotFoundException, DoesntExistException 
    {
        ...
        Comment comment=comObjector.getCommentObject(dataObject);
        CommunicationSQLCommands comSQL=new CommunicationSQLCommands(connection);
        comSQL.addComment(comment.getProductId(), user.getUser().getUserId(), comment.getComment());

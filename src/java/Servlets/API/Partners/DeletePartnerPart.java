 ...
 * @author Moses
 */
public class DeletePartnerPart extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet DeletePartnerPart</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet DeletePartnerPart at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
            ...
            throws ServletException, IOException, SQLException, ClassNotFoundException, DoesntExistException 
    {
        ...
        int partId=dataObject.optInt("partId");
        PartnerSQLCommands partnerSQL=new PartnerSQLCommands(connection);
        int cartId=partnerSQL.getEmptyCartId(partnerSafe.getPartner().getPartnerId());
        partnerSQL.deletePartnerPart(partId, cartId);

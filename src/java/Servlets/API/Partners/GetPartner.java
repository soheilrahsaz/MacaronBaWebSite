...
public class GetPartner extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet GetPartner</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet GetPartner at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
...
//        }
        ...
        LinkedList<UserAddress> addresses=partnerSQL.getPartnerAddresses(partnerSafe.getPartner().getPartnerId());
        UserJsoner userJsoner=new UserJsoner();
        PartnerJsoner partnerJsoner=new PartnerJsoner(connection);
        JSONObject partnerObject=partnerJsoner.getPartnerJson(partnerSafe.getPartner());
        JSONArray partnerAddresses=userJsoner.getAddressArray(addresses);
        JSONObject responseObject=new JSONObject();
        responseObject.put("partner", partnerObject);
        responseObject.put("address", partnerAddresses);
        out.println(responser.response(responseObject, 1));

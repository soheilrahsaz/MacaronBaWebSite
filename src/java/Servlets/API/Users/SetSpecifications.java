 ...
 * @author Moses
 */
public class SetSpecifications extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet SetSpecifications</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet SetSpecifications at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
            ...
            throws ServletException, IOException, SQLException, ClassNotFoundException, DoesntExistException 
    {
        ...
        JSONObject specificData=new JSONObject(data);
        UserSQLCommands userSQL=new UserSQLCommands(connection);
        String email=specificData.optString("email");
        String firstName=specificData.optString("firstName");
        String lastName=specificData.optString("lastName");
        long birthDate=specificData.optLong("birthDate");
        if(birthDate==0)
        {
            userSQL.setSpecificatoins(user.getUser().getUserId(),email, firstName,lastName);
        }else
        {
            userSQL.setSpecificatoins(user.getUser().getUserId(),email, firstName,lastName, 
                new Date(birthDate));

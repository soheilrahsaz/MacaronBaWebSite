 ...
 */
@MultipartConfig
public class AddScale extends HttpServlet {
...
private final String URL_LIST="/AdminPanel/Products/List/";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet AddScale</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet AddScale at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        {
            doItdoPost(request, response, connection);
        } catch (Exception e) {
    ...
    public void doItdoPost(HttpServletRequest request, HttpServletResponse response,Connection connection)
            throws ServletException, IOException, SQLException, ClassNotFoundException, DoesntExistException
    {
        ...
        String scaleValue=request.getParameter("value");
        String effect=request.getParameter("effect");
        
        if(Methods.Methods.isNullOrEmpty(effect,scaleValue))
        {
           response.sendRedirect(URL_LIST+product+"?att=empty");
           return;
        }
        double value=Double.parseDouble(scaleValue);
        double effectVal=Double.parseDouble(effect);
        
        if(!Methods.Methods.isNumberDouble(effect)&&!Methods.Methods.isNumberDouble(scaleValue))
        {
           response.sendRedirect(URL_LIST+product+"?att=number");
           return; 
        }
        
        if(productSQL.scaleValueExist(scaleId,value))
        {
            response.sendRedirect(URL_LIST+product+"?att=exists");
            return; 
        }
       ...
       productSQL.addScaleValue(scaleId, value,effectVal,description,picture);

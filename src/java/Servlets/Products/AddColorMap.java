...
private final String URL_COLOR_MAP="ColorMap.jsp";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet AddColorMap</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet AddColorMap at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
            ...
            throws ServletException, IOException, SQLException, ClassNotFoundException, DoesntExistException
    {
        ...
        if(Methods.Methods.isNullOrEmpty(taste,color))
        {
            response.sendRedirect(URL_COLOR_MAP+"?att=empty");
            return;
        }
        ProductSQLCommands productSQL=new ProductSQLCommands(connection);
        productSQL.addColorMap(taste, color);
        response.sendRedirect(URL_COLOR_MAP+"?att=ok");
        return;

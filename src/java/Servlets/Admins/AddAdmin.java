...
private final String URL_ADMIN_LIST="List/";
            ...
            throws ServletException, IOException, SQLException, ClassNotFoundException, NoSuchAlgorithmException, DoesntExistException
    {
        ...
        {
            response.sendRedirect(URL_ADD_ADMIN+"?att=empty");
            return;
        }
            
        LinkedList<String> access=new LinkedList<String>();
        for(String str:new AdminAccess().getAdminAccess().keySet())
        {
            String acc=""+request.getParameter(str);
            if(acc.equalsIgnoreCase("on"))
            {
                access.add(str);
            }
        }
        ...
        int accessId=adminSQL.addAccess(access);
        int adminId=adminSQL.addAdmin(userName, passwordHashed, accessId);

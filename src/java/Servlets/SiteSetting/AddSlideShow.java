...
private final String URL_SLIDE_SHOWS="SlideShow.jsp";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet AddSlideShow</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet AddSlideShow at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
            ...
            throws ServletException, IOException, SQLException, ClassNotFoundException, DoesntExistException
    {
        ...
        if(!filename.isEmpty())
        {
            ...
            String path=Methods.Methods.mkdirForSlideShow(getServletContext().getInitParameter("SlideShows"))+picture;
        ...
        {
            response.sendRedirect(URL_SLIDE_SHOWS+"?att=empty");
            return;
        }
        
        SiteSettingSQLCommands siteSQL=new SiteSettingSQLCommands(connection);
        siteSQL.addSlideShow(link, text, picture);
        response.sendRedirect(URL_SLIDE_SHOWS+"?att=ok");

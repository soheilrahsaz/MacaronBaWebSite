...
private final String URL_UPLOAD="Upload.jsp";
    /**
            ...
            throws ServletException, IOException, SQLException, ClassNotFoundException, DoesntExistException
    {
        ...
        Video video=siteSQL.getVideo(videoId);
        siteSQL.deleteVideo(videoId);
        
        try 
        {
            Files.deleteIfExists(Paths.get(System.getProperty("catalina.home")+"/"+getServletContext().getInitParameter("Videos")+"/"+video.getUrl()));
        } catch (Exception e) {
        }
        
        response.sendRedirect(URL_UPLOAD+"?att=ok");
        return;

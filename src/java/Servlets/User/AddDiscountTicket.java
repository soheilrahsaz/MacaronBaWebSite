 ...
 * @author Moses
 */
public class AddDiscountTicket extends HttpServlet {
private final String URL_INDEX="../index.jsp";
private final String URL_LOGIN="../Login.jsp";
private final String URL_USER_SHOW="List/";
    /**
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet AddDiscountTicket</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet AddDiscountTicket at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
            ...
            throws ServletException, IOException, SQLException, ClassNotFoundException, DoesntExistException
    {
        ...
        if(!admin.haveAccessTo("editUsers"))
        {
        ...
        String dateAlt=request.getParameter("dateAlt");
        
        if(Methods.Methods.isNullOrEmpty(dateAlt,effect))
        {
            response.sendRedirect(URL_USER_SHOW+userid+"?att=empty");
            return;
        }
        
        double effectVal=Double.parseDouble(effect);
        long time=Long.parseLong(dateAlt);
        time*=1000;
        Timestamp timestamp=new Timestamp(time);
        
        OrderHistorySQLCommands sql=new OrderHistorySQLCommands(connection);
        sql.addDiscount(userId, effectVal, timestamp);
        response.sendRedirect(URL_USER_SHOW+userid+"?att=ok");
            return;
    }

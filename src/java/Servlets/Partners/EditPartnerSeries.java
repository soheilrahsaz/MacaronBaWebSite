 ...
 * @author Moses
 */
public class EditPartnerSeries extends HttpServlet {
private final String URL_INDEX="/AdminPanel/index.jsp";
private final String URL_ADMIN_LOGIN="../Login.jsp";
private final String URL_EDIT_SERIES="SeriesList/";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet EditPartnerSeries</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet EditPartnerSeries at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
            ...
            throws ServletException, IOException, SQLException, ClassNotFoundException, DoesntExistException, NoSuchAlgorithmException
    {
        ...
        String id=request.getParameter("seriesId");
        int idVal=Integer.parseInt(id);
        if(Methods.Methods.isNullOrEmpty(name,effect))
        {
            response.sendRedirect(URL_EDIT_SERIES+id+"?att=empty");
            return;
        }
        double effectVal=Double.parseDouble(effect);
        
        PartnerSQLCommands partnerSQL=new PartnerSQLCommands(connection);
        partnerSQL.updatePartnerSeries(idVal, name, effectVal);
        response.sendRedirect(URL_EDIT_SERIES+id+"?att=ok");
            return;
